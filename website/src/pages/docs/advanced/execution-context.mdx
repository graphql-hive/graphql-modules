...
<Callout>
  Execution context uses the `async_hooks` module, and therefore `@ExecutionContext` impacts the performance.

GraphQL Modules is smart enough to enable `async_hooks` only when
`@ExecutionContext` is used though.

Note that `async_hooks` is important to avoid race conditions concurrent parallel requests.
